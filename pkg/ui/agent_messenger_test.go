...
// TestAgentMessenger_FormatContent tests content formatting
func TestAgentMessenger_FormatContent(t *testing.T) {
	injector := do.New()
	...
	tests := []struct {
	...
	}{
		{
			...
			contains: []string{"test response"},
		},
		{
			name: "response with role",
			content: map[string]any{
				"response":   "test response",
				"agent_role": "Supervisor",
			},
			contains: []string{"test response", "Role: Supervisor"},
		},
		{
			name: "error message",
			content: map[string]any{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := impl.formatContent(tt.content)
