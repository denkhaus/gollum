...
//
//	mockgen -source=pkg/channel/handler.go -destination=pkg/channel/handler_input_mock.go -package=channel
//
...
func (m *MockInputHandler) HandleInput(session *shared.Session, input string) (*InputResult, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "HandleInput", session, input)
...
func (mr *MockInputHandlerMockRecorder) HandleInput(session, input any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "HandleInput", reflect.TypeOf((*MockInputHandler)(nil).HandleInput), session, input)
}
