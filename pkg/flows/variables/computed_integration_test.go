...
func TestComputedField_DependencyExtraction(t *testing.T) {
	...
	assert.Equal(t, flows.FlowVariableScopeInput, field.Dependencies[0].Scope)
	...
	for _, dep := range field3.Dependencies {
		if dep.Scope == flows.FlowVariableScopeInput {
			hasInputDep = true
		}
		if dep.Scope == flows.FlowVariableScopeContext {
			hasContextDep = true
		}
	}
...
func TestComputedField_ComputedToComputedDependencies(t *testing.T) {
	...
	assert.Equal(t, flows.FlowVariableScopeInput, field1.Dependencies[0].Scope)

	// Verify doubled depends on computed.base
	field2, ok := cv.fields["doubled"]
	assert.True(t, ok)
	assert.Len(t, field2.Dependencies, 1)
	assert.Equal(t, flows.FlowVariableScopeComputed, field2.Dependencies[0].Scope)
	...
	assert.Equal(t, flows.FlowVariableScopeComputed, field3.Dependencies[0].Scope)
...
func TestComputedField_MultipleScopeDependencies(t *testing.T) {
	...
	assert.True(t, ok)
	assert.Len(t, field3.Dependencies, 1)
	assert.Equal(t, flows.FlowVariableScopeOutput, field3.Dependencies[0].Scope)
}
...
func TestComputedField_CircularDependencyDetection(t *testing.T) {
	...
	assert.Equal(t, flows.FlowVariableScopeComputed, fieldA.Dependencies[0].Scope)
}
